...
class AXEL_FakeStakeTest(BitcoinTestFramework):
    ...
    def spend_utxos(self, utxo_list, address_list = []):
        ...
        :param      utxo_list:      (JSON list) returned from listunspent used as input
                    address_list:   (string list) [optional] recipient AXEL addresses. if not set,
    ...
    def stake_amplification_step(self, utxo_list, address_list = []):
        ...
        :param      utxo_list:     (JSON list) returned from listunspent used as input
                    address_list:  (string list) [optional] recipient AXEL addresses.
    ...
    def test_spam(self, name, staking_utxo_list,
        ...
        for i in range(0, self.NUM_BLOCKS):
            ...
            time.sleep(1)
            if (not fMustPass and var not in [None, "bad-txns-invalid-zpiv", "rejected"]) or (fMustPass and var != "inconclusive"):
