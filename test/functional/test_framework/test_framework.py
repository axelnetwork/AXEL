...
class PivxTestFramework():
    """Base class for a axel test script.

    Individual axel test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave axelds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop axelds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing axeld/axel-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use axel-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: axelds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a axeld test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple axeld test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2 ** 16) as log_stderr:
            try:
                ...
                assert 'axeld exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "axeld should have exited with an error"
                else:
                    assert_msg = "axeld should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as axeld's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in axel.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                for subdir in ["blocks", "chainstate"]:  # CHANGED for AXEL
                    copy_and_overwrite(os.path.join(node_0_datadir, subdir),
                                    os.path.join(node_i_datadir, subdir))
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in axel.conf
        ...
        def clean_cache_subdir(cachedir):
            ...
            for i in range(MAX_NODES):
                for entry in os.listdir(cache_path(i)):
                    if entry not in ['wallet.dat', 'chainstate', 'blocks', 'backups']:  # CHANGED for AXEL
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "axeld"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run axelds:
    ...
    def test_PoS_chain_balances(self):
        ...
        assert_equal(w_info[0]["balance"], DecimalAmt(1500000.0 * (50 + 12 + 20)))      # CHANGED for AXEL
        assert_equal(w_info[1]["balance"], DecimalAmt(1500000.0 * (50 + 12 + 20)))      # CHANGED for AXEL
        assert_equal(w_info[2]["balance"], DecimalAmt(1500000.0 * (50 + 12 + 20)))      # CHANGED for AXEL
        assert_equal(w_info[3]["balance"], DecimalAmt(1500000.0 * (50 + 12 + 20 + 2)))  # CHANGED for AXEL
    ...
    def generate_pos(self, node_id, btime=None):
        ...
        assert ss["mintablecoins"]  # CHANGED for AXEL
        assert ss["enoughcoins"]    # CHANGED for AXEL
        ...
        while not fStaked:
            try:
                ...
                if ("Couldn't create new block" in str(e)):
                    ...
                    if failures > 60:
                        ss = rpc_conn.getstakingstatus()
                        if not ss["walletunlocked"]:  # CHANGED for AXEL
...
class ComparisonTestFramework(PivxTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "axeld"),
                          help="axeld binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "axeld"),
                          help="axeld binary to use for reference nodes (if any)")
