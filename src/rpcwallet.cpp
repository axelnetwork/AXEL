...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new axel address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"axeladdress\"    (string) The new axel address\n"
            "\nExamples:\n" +
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current axel address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"axeladdress\"   (string) The account axel address\n"
            "\nExamples:\n" +
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new axel address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"axeladdress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"axeladdress\"  (string, required) The axel address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"axeladdress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"axeladdress\"  (string, required) The axel address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"axeladdress\"  (string) a axel address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"axeladdress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"axeladdress\"  (string, required) The axel address to send to.\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddressix \"axeladdress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"axeladdress\"  (string, required) The axel address to send to.\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"axeladdress\",     (string) The axel address\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"axeladdress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"
                                        "\nArguments:\n"
                                        "1. \"axeladdress\"  (string, required) The axel address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"axeladdress\" ( minconf )\n"
            "\nReturns the total amount received by the given axeladdress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"axeladdress\"  (string, required) The axel address for transactions.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toaxeladdress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a axel address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"
                                        "\nArguments:\n"
                                        "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                                        "2. \"toaxeladdress\"  (string, required) The axel address to send funds to.\n"
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The axel address is the key, the numeric amount in btc is the value\n"
                                        "      ,...\n"
                                        "    }\n"
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "Each key is a axel address or hex-encoded public key.\n"
                     "If 'account' is specified, assign address to that account.\n"

                     "\nArguments:\n"
                     "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
                     "2. \"keysobject\"   (string, required) A json array of axel addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"address\"  (string) axel address or hex-encoded public key\n"
                     "       ...,\n"
                     "     ]\n"
                     "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

                     "\nResult:\n"
                     "\"axeladdress\"  (string) A axel address associated with the keys.\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"axeladdress\",    (string) The axel address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"axeladdress\",    (string) The axel address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"axeladdress\",   (string) The axel address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending axels\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending axels\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"axeladdress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; axel server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending axels.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in axel/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total axel balance of the wallet\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same axel address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <axel Address> <percent>\n"
