...
// Copyright (c) 2017-2018 The axel developers
...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or axel.conf setting gen)\n"
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if(fPoS){
            if (!pblock->SignBlock(*pwalletMain)) {
                LogPrintf("generate: Signing new block failed \n");
            }
        }
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "AXEL node is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "AXEL node is downloading blocks...");
