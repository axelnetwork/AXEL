...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"axelprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"axelprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AXEL address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"axeladdress\"\n"
            "\nReveals the private key corresponding to 'axeladdress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"axeladdress\"   (string, required) The AXEL address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by AXEL %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"axeladdress\"\n"
            "\nEncrypts a private key corresponding to 'axeladdress'.\n"
            "\nArguments:\n"
            "1. \"axeladdress\"   (string, required) The AXEL address for the private key (you must hold the key already)\n"
