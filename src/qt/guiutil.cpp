...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AXEL address (e.g. %1)").arg("eSBxEjGXmJxwpXxXAJqeXztdUR4MsZZJsE"));
#endif
...
{
    // return if URI is not valid or is no axel: URI
...
{
    // Convert axel:// to axel:
    //
    //    Cannot handle this later, because axel:// will cause Qt to see the part after // as host,
...
bool isDust(const QString& address, const CAmount& amount)
{
    ...
    return txOut.IsDust(SelectMinRelayTxFee());
}

CAmount getDust()
{
  return 3*SelectMinRelayTxFee().GetFee(182);
}
...
void openConfigfile()
{
    ...
    /* Open axel.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "axel.lnk";
}
...
{
    return GetAutostartDir() / "axel.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a axel.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AXEL wallet\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the axel app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add axel app to startup item list
