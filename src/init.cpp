...
//    strUsage += HelpMessageOpt("-gmnotify=<cmd>", _("Execute command when a gm message is received (%s in cmd is replaced by message)"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 32323, 42323));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in axel/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::SelectMinTxFee().GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in axel/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(SelectMinRelayTxFee().GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 32325, 42325));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2014 The Bitcoin Core Developers")) + "\n" +
	   ...
	   "\n" +
           ...
           "\n" +
           FormatParagraph(_("Terms & Policies:<https://www.axel.org/terms-and-policies/>")) +
           "\n";
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-mintxfee")) {
        ...
        if (ParseMoney(mapArgs["-mintxfee"], n) && n > 0)
            CWallet::SetMinTxFee(CFeeRate(n));
        else
